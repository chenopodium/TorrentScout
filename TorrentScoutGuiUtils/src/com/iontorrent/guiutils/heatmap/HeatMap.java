 ...
 * @version 1.6
 * @author Chantal Roth (modified)
 */
public class HeatMap extends JPanel {

    protected double[][] data;
    private int[][] dataColorIndices;
    private boolean symmetricalRange = false;
    private int BX = 60;
    private int BY = 30;
    private int DY = 30;
    private int DX = 30;
    ...
    private int nrXTicks;

    public int getNrXTicks() {
        return nrXTicks;
    }

    public void setNrXTicks(int nrXTicks) {
        this.nrXTicks = nrXTicks;
    }

    public int getNrYTicks() {
        return nrYTicks;
    }

    public void setNrYTicks(int nrYTicks) {
        this.nrYTicks = nrYTicks;
    }
    private int nrYTicks;
    ...
    private boolean drawLegendText = true;
    private boolean drawXTicks = false;
    private boolean drawYTicks = false;
    private boolean drawDiag = false;
    ...
    private void updateDataColors() {
        ...
        if (this.isSymmetricalRange() && smallest < 0 && largest > 0) {
            // largst = -smallest
            double max = Math.max(largest, -smallest);
            largest = max;
            smallest = -max;
        }
    ...
    protected void drawData() {
        ...
        for (int x = 0; x < data.length; x++) {
            for (int y = 0; y < data[0].length; y++) {
                int ind = dataColorIndices[x][y];
                bufferedGraphics.setColor(colors[ind]);
                bufferedGraphics.fillRect(x, data[0].length - y, 1, 1);
            }
        }
    }
    ...
    public void paintComponent(Graphics g) {
        ...
        // border
        g2d.setColor(fg);
        g2d.drawRect(DX, DY - 5, width - DX - BX, height - DY - BY);

        // title
        if (drawTitle && title != null) {
            g2d.setFont(new Font("Helvetica", Font.BOLD, 18));
            g2d.drawString(title, (width / 2) - 4 * title.length(), 14);
            g2d.setFont(new Font("Helvetica", Font.PLAIN, 12));

        }

        // axis ticks - ticks start even with the bottom left coner, end very close to end of line (might not be right on)
        int numXTicks = (width - BX - DX) / 50;
        int numYTicks = (height - DY - BY) / 50;
        if (this.getNrXTicks() > 0) {
            numXTicks = this.getNrXTicks();
        }
        if (this.getNrYTicks() > 0) {
            numYTicks = this.getNrYTicks();
        }
        String label = "";
        ...
        // Y-Axis ticks
        if (drawYTicks) {
            int yDist = (int) ((height - DY - BY) / (double) numYTicks); //distance between ticks
            for (int y = 0; y <= numYTicks; y++) {
                g2d.drawLine(DX - 4, height - DY - y * yDist, DY, height - BY - y * yDist);
                label = df.format(((y / (double) numYTicks) * (yMax - yMin)) + yMin);
                int labelY = height - DY - y * yDist - 4 * label.length();
        ...
        // X-Axis ticks
        if (drawXTicks) {
            int xDist = (int) ((width - DX - BX) / (double) numXTicks); //distance between ticks
            for (int x = 0; x <= numXTicks; x++) {
                g2d.drawLine(DX + x * xDist, height - DY, DX + x * xDist, height - DY - 4);
                label = df.format(((x / (double) numXTicks) * (xMax - xMin)) + xMin);
                int labelX = (DX + 1 + x * xDist) - 4 * label.length();
        ...
        // Legend
        if (drawLegend) {
            g2d.drawRect(width - BX + 10, DY, LX, height - DY - BY);
            for (int y = 0; y < height - 61; y++) {
                int yStart = height - DY - 1 - (int) Math.ceil(y * ((height - DY - BY) / (colors.length * 1.0)));
                yStart = height - DY - 1 - y;
                g2d.setColor(colors[(int) ((y / (double) (height - DY - BY)) * (colors.length * 1.0))]);
                g2d.fillRect(width - BX + 11, yStart, LX - 1, 1);
            }
            if (drawLegendText) {
                int x = width - BX + LX + 12;
                g2d.setColor(Color.black);
                g2d.drawString("" + smallest, x, height - BY + 4);
                g2d.drawString("" + largest, x, DY + 5);
                int y = (height - BY - 1 + DY) / 2;
                g2d.drawString("" + (largest + smallest) / 2, x, y + 4);
                g2d.drawLine(x - LX, y, x - 1, y);
            }
        }
        if (drawDiag) {
            g2d.setColor(Color.black);
            g2d.drawLine(DX, height - BY,
                    width - BX,
                    DY);
        }
    }

    public boolean isDrawLegendText() {
        return drawLegendText;
    }

    public boolean isSymmetricalRange() {
        return symmetricalRange;
    }

    public void setSymmetricalRange(boolean symmetricalRange) {
        this.symmetricalRange = symmetricalRange;
    }

    public void setDrawLegendText(boolean drawLegendText) {
        this.drawLegendText = drawLegendText;
    }

    public boolean isDrawDiag() {
        return drawDiag;
    }

    public void setDrawDiag(boolean drawDiag) {
        this.drawDiag = drawDiag;
    }
    ...
    private void err(String msg, Exception ex) {
        Logger.getLogger(HeatMap.class.getName()).log(Level.SEVERE, msg, ex);
    }

    private void p(String msg) {
        System.out.println("HeatMap: " + msg);
        //Logger.getLogger( HeatMap.class.getName()).log(Level.INFO, msg, ex);
    }
}
