 ...
 * @author Chantal Roth
 */
public class MultiFlowPanel extends JPanel implements ProgressListener{
    ...
    public MultiFlowPanel(ExperimentContext expContext, MultiFlowLoader loader) {
    ...
    private void udpateChart() {
        if (chart != null) chart.update(loader.getMultiflowresults(), subtract);
    }
    private void updateChart(WellData welldata, int subtract) {
        if (scroll != null) {
            remove(scroll);
        }
        if (welldata == null) {
       ...
       // if (loader.getMultiflowresults()== null || loader.getMultiflowresults()[0] == null) loader.compute(flowpanel.getFlows(), this);
        //  if (welldata.getAverageFlowValue() > 0) {
        chart = new MultiFlowChartPanel(new Ionogram(welldata, context), raw, norm, loader.getMultiflowresults(), subtract, minflow, maxflow);
        chart.setMaxy(maxy);
