...
//    }
    ...
    public void clearData(BfMaskFlag flag) {
        int nrflags = BfMaskFlag.getNrFlags();
        if (done == null) {
    ...
    public int[][] readData(BfMaskFlag flag, String file) {
        return readData(flag, file, false);
    }

    public int[][] readData(BfMaskFlag flag, String file, boolean forceRead) {
        int nrflags = BfMaskFlag.getNrFlags();
        ...
        if (!forceRead && done[flag.getCode()]) {
            p("Already read data for flag " + flag);
        ...
        p("read image " + getFile(flag) + ", extracting data from it. Image size=" + image.getData().getBounds());
...
//            }
    ...
    public int getDataPointAt(BfMaskFlag flag, int col, int row, boolean debug) {
        ...
        if (which >= alldata.length) {
            which = 0;
        }

        if (col >= alldata[which].length) {
            if (debug) {
                p("Trying to get alldata[" + col + "][" + row + "], but max c is " + alldata[which].length + ", which=" + which);
            }
            // err("Tyring to get alldata[" + col + "][" + row + "], but max c is " + alldata[0].length);
            return 0;

        } else if (row >= alldata[which][col].length) {
            if (debug) {
                p("Tyring to get alldata[" + col + "][" + row + "], but max r is " + alldata[which][col].length + ", which=" + which);
            }
            return 0;
        }
