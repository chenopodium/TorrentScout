...
// one for each frame
        ...
        for (WellFlowDataResult dataresult : results) {
            ...
            String serieskey = "flow " + dataresult.getFlow();
            if (base != null) {
                serieskey += " " + base;
            }
    ...
    private JFreeChart createChart(XYDataset dataset) {
        ...
        chart = ChartFactory.createXYLineChart(region, // chart
                // title
        ...
        for (int s = 0; s < series; s++) {
            String key = dataset.getSeriesKey(s).toString();
            Paint c = renderer.getSeriesOutlinePaint(s);
            if (key.endsWith(" T")) {
                c = Color.red;
            ...
            XYTextAnnotation text = null;
            int count = dataset.getItemCount(s);
            if (count > 0) {
                int x = dataset.getX(s, count / 2).intValue();
                int y = -10;//dataset.getY(s, 0).intValue();
                text = new XYTextAnnotation(key, x, y);
                text.setPaint(c);
